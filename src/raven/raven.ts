...
class Raven extends TypedEventEmitter<RavenEvents, EventHandlerMap> {
    ...
    static isSpam(content: string) {
        ...
        return match(emojiRe).length > 2            // at least 3 emojis
            && match(/\n/g).length > 1          // at least 2 new lines
            && match(/https?:\/\//g).length > 0 // at least 1 link
    }
}
